...
/**
 * @link              https://github.com/askinne2/Shelterluv-Animals
 * @since             1.0.0
 * @package           [shelterluv_animals]
 *
 * @wordpress-plugin
 * Plugin Name:       Shelterluv Animals
 * Plugin URI:        https://github.com/askinne2/Shelterluv-Animals
 *
 * Description:       Displays all animals that are currently listed in Shelterluv database on website as Animal Post Type.
 *
 *
 * Version:           2.4.1
 ...
 * Text Domain:       shelterluv_animals
 * Domain Path:       /languages
 * GitHub Plugin URI: https://github.com/askinne2/Shelterluv-Animals
...
define('SL_UPLOADS', 'wp-content/uploads/Shelterluv-Animals');
define('ADOPT_LINK', 'https://www.shelterluv.com/matchme/adopt/ghhs-a-');

require_once 'shelterluv_animals_includes.php';
require_once 'shelterluv_animals_slideshow.php';
require_once 'shelterluv_animals_acf.php';
require_once 'includes/smush-integration.php';
require_once 'includes/acf-pro/acf.php';
...
*/
	public function __construct()
	{
		...
		// set up request arguments (API Key &  Animal Statuses)
	...
	public function set_request_args()
	{
		...
		$this->shelterluv_animals = new Shelterluv_Animals_ACF();

		$this->args = array(
			'headers' => array(
				//'x-api-key' => '7a8f9f04-3052-455f-bf65-54e833f2a5e7',
				'x-api-key' => get_field('api_key', 'option')
			),
		);
		if ($this->args['headers']['x-api-key'] == NULL) {
			echo "<h5>Website cannot connect to Shelterluv server. API Key needed.</h5>";
			return ob_get_clean();
		}

		$this->status_array['status1'] = get_field('animal_status1', 'option');
		if (!$this->status_array['status1']) {
			echo "<h5>Please set statuses for animals to select from Shelterluv</h5>";
			return ob_get_clean();
		}
		$this->status_array['status2'] = get_field('animal_status2', 'option');
		if (!$this->status_array['status2']) {
			echo "<h5>Please set statuses for animals to select from Shelterluv</h5>";
			return ob_get_clean();
		}
		$this->status_array['status3'] = get_field('animal_status3', 'option');
		if (!$this->status_array['status3']) {
			echo "<h5>Please set statuses for animals to select from Shelterluv</h5>";
			return ob_get_clean();
		}
		$this->status_array['status4'] = get_field('animal_status4', 'option');
		if (!$this->status_array['status4']) {
			echo "<h5>Please set statuses for animals to select from Shelterluv</h5>";
			return ob_get_clean();
		}
		return ob_get_clean();
	...
	public function make_local_request()
	{
		...
		if (!empty($transient)) {
			...
			$jsonpets = file_get_contents(plugins_url(plugin_basename(__DIR__)) . '/example-animal-json/animals.json');
	...
	{
		$pet_slideshow = new Shelterluv_Animals_Slideshow();
		$pet_slideshow->run();
	}
		...
		*/
		...
		if ($posts) {
			...
			foreach ($posts as $post) {
				$animal_ids->shelterluv_id[$i] = get_field('animal_id', $post->ID);
		...
		$i = 0;
		foreach ($animal_ids->shelterluv_id as $id) {
	...
	*/
	...
	public function create_animal_post($animal)
	{
		...
		if ($new_post_id) {
			...
			$birthday->setTimestamp($animal->DOBUnixTime);
			$now = new DateTime();
			$temp_age = $now->diff($birthday);
			$age_string = sprintf("%d years, %s months", $temp_age->y, $temp_age->m);
			if (PLUGIN_DEBUG) {
				printf('<h5 class="red_pet">creating Age for %s is: %s</h5>', $animal->Name, $age_string);
			}

			add_post_meta($new_post_id, 'age', $age_string);
	...
	public function update_animal($animal, $post_id)
	{
		...
		if (PLUGIN_DEBUG) {
			printf('<h5>Pet: %s</h5><h4>Age: %s months</h4>', $animal->Name, $animal->Age);

			printf('<h4 class="red_pet">Post Update Time: %s </h4>', $postUpdateTime);
		...
		$age_string = sprintf("%d years, %s months", $temp_age->y, $temp_age->m);
			if (PLUGIN_DEBUG) {
				printf('<h5 class="red_pet">Age for %s is: %s</h5>', $animal->Name, $age_string);
			}
		...
		if ($animal->LastUpdatedUnixTime >= $postUpdateTime) {
			...
			update_post_meta($post_id->ID, 'age', $age_string);
	...
	public function shelterluv_archive_animal_template($template)
	{
	...
	public function shelterluv_single_animal_template($template)
	{
	...
	public function run_update()
	{
		...
		if (PLUGIN_DEBUG) {
			printf('<h2>API Key: %s</h2>', $this->args['headers']['x-api-key']);
		}
...
register_activation_hook(__FILE__, array('Shelterluv_Animals', 'activate'));
register_deactivation_hook(__FILE__, array('Shelterluv_Animals', 'deactivate'));

// run Shelterluv_Animals shortcode
