...
class LGL_Utilities {
     ...
     */
    public static function get_filtered_orders($start_datetime, $end_datetime, $statuses = ['completed']) {
        ...
        if (!($start_datetime instanceof DateTime) || !($end_datetime instanceof DateTime)) {
            error_log('LGL Utilities: Invalid datetime parameters provided to get_filtered_orders()');
            return [];
        }
        ...
        $cached_orders = LGL_Cache_Manager::get($cache_key);
        ...
        try {
            ...
            LGL_Cache_Manager::set($cache_key, $orders, 3600);
    ...
    public static function format_order_data($order) {
        if (!is_object($order) || !method_exists($order, 'get_id')) {
            error_log('LGL Utilities: Invalid order object provided to format_order_data()');
            return null;
        }
     ...
     */
    public static function get_user_lgl_id($user_id) {
     ...
     */
    public static function log_environment_info() {
...
if (!function_exists('get_filtered_orders')) {
    function get_filtered_orders($start_datetime, $end_datetime) {
        return LGL_Utilities::get_filtered_orders($start_datetime, $end_datetime);
    }
}

if (!function_exists('format_order_data')) {
    function format_order_data($order) {
        return LGL_Utilities::format_order_data($order);
    }
}
